...
import { FaExclamationCircle, FaUser, FaUsers, FaArrowRight, FaTicketAlt } from 'react-icons/fa';
...
const ConvertToTicketForm: React.FC<ConvertToTicketFormProps> = ({ 
  ...
  const filteredUsers = () => {
    if (!formData.assignToTeam || !users) return [];
    
    // Find the selected team
    const selectedTeam = teams?.find(team => team._id === formData.assignToTeam);
    if (!selectedTeam) return [];
    
    // Filter users who are members of the selected team
    return users.data.filter(user => {
      return selectedTeam.members?.some(member => member.userId._id === user._id);
    });
  };
      ...
      <div className="space-y-4">
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 mb-4">
          ...
          <p className="text-gray-300 text-sm">
            This will create a new ticket based on this query. The query will be marked as converted and linked to the new ticket.
          </p>
        </div>
        ...
        <div className="border border-gray-700/50 rounded-lg p-4 bg-gray-800/30">
          ...
          <div>
            ...
            <div className="text-white">
              {query.customer?.userId?.profile?.firstName} {query.customer?.userId?.profile?.lastName} ({query.customer?.userId?.email})
            </div>
          </div>
        </div>
        ...
        {/* Team Assignment */}
        <div>
          <label htmlFor="assignToTeam" className="block text-sm font-medium text-gray-300 mb-1">
          ...
          <div className="relative">
            <select
              ...
              className={`w-full bg-gray-700/50 border ${errors.assignToTeam ? 'border-red-500' : 'border-gray-600/50'} rounded-lg py-2 px-4 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none`}
            >
              <option value="">Select Team</option>
              {teams?.filter(team => team.teamType === 'technical').map(team => (
        ...
        {/* User Assignment */}
        <div>
          <label htmlFor="assignToUser" className="block text-sm font-medium text-gray-300 mb-1">
            Assign to Team Member <span className="text-gray-500">(Optional)</span>
          </label>
          <div className="relative">
            <select
              id="assignToUser"
              ...
              onChange={handleChange}
              disabled={!formData.assignToTeam || filteredUsers().length === 0}
          ...
          {formData.assignToTeam && filteredUsers().length === 0 && (
            <p className="mt-1 text-sm text-amber-500 flex items-center">
              <FaExclamationCircle className="mr-1" /> No team members available for the selected team
            </p>
          )}
        </div>
