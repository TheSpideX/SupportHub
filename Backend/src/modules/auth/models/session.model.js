...
const SessionSchema = new mongoose.Schema(
  {
    ...
    deviceInfo: {
      ...
      // IMPROVEMENT: Add fingerprint for better device tracking
      fingerprint: String,
      // IMPROVEMENT: Add additional device identifiers
      screenResolution: String,
      timezone: String,
      language: String,
    },
    // IMPROVEMENT: Add security metadata for better tracking
    securityMetadata: {
      createdAt: Date,
      ipAddress: String,
      geoLocation: {
...
SessionSchema.methods.removeTab = async function (tabId) {
  ...
  const sessionRoom = await Room.findOne({ roomId: this.getRoomId() });
  if (sessionRoom) {
    sessionRoom.childRooms = sessionRoom.childRooms.filter(
      (id) => id !== tabRoomId
    );
    await sessionRoom.save();
  }
...
SessionSchema.methods.propagateEvent = async function (
  eventType,
  eventData,
  direction = "down"
) {
  ...
  if (direction === "up") {
    ...
    const deviceRoom = await Room.findOne({
      roomId: this.hierarchy.deviceRoom,
    });
...
SessionSchema.methods.subscribeToRoom = async function (roomId) {
  ...
  const isSubscribed = this.roomSubscriptions.some(
...
SessionSchema.methods.unsubscribeFromRoom = async function (roomId) {
  // Check if subscribed
  const subscriptionIndex = this.roomSubscriptions.findIndex(
    (sub) => sub.roomId === roomId
  );
  ...
  if (subscriptionIndex !== -1) {
    ...
    if (room && this.socketIds.length > 0) {
      room.members = room.members.filter(
        (member) => !this.socketIds.includes(member)
      );
