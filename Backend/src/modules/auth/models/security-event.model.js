...
const securityEventSchema = new mongoose.Schema({
  ...
  organizationId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: "Organization",
    required: false, // Not required for events without organization context
    index: true,
  },
  eventType: {
    type: String,
...
securityEventSchema.index({ organizationId: 1, eventType: 1, timestamp: -1 });
...
securityEventSchema.methods.shouldPropagateUp = function () {
  ...
  return (
    upwardEvents.includes(this.eventType) ||
...
securityEventSchema.methods.shouldPropagateDown = function () {
  ...
  return (
    downwardEvents.includes(this.eventType) ||
