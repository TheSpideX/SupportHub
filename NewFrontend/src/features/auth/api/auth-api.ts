    ...
    if (
      ...
      if (refreshQueueService.isRefreshInProgress()) {
        try {
          ...
          window.location.href = "/auth/login"; // Updated from '/auth/login'
...
export const authApi = {
  ...
  validateSession: async () => {
    try {
      ...
      if (response.data && response.data.authenticated) {
        ...
        const detailsResponse = await apiInstance.get(
  ...
  login: async (credentials) => {
    try {
      ...
      // Get stored deviceId or generate a new one
      const deviceId =
      ...
      // Generate a unique tabId for this browser tab
      const tabId =
        sessionStorage.getItem("tab_id") ||
        `tab-${Date.now()}-${Math.random().toString(36).substring(2, 15)}`;

      // Store tabId in sessionStorage (will be cleared when tab is closed)
      sessionStorage.setItem("tab_id", tabId);

      // Add device info to credentials
      const requestData = {
        ...credentials,
        deviceId: deviceId, // Send deviceId as a top-level property
        tabId: tabId, // Send tabId as a top-level property
        deviceInfo: {
          deviceId: deviceId, // Also include in deviceInfo for backward compatibility
          tabId: tabId, // Also include tabId in deviceInfo
          fingerprint,
  ...
  getActiveSessions: async () => {
    try {
      const csrfToken =
        tokenService.getCsrfToken() || localStorage.getItem("csrf_token") || "";
    ...
    try {
      ...
      if (response.data?.sessionId && response.data?.expiresAt) {
        const sessionData = {
          ...
          warningAt: response.data.warningAt
            ? new Date(response.data.warningAt).getTime()
            : null,
        };

        // Store updated session data
        localStorage.setItem(
          API_CONFIG.AUTH.SESSION.STORAGE_KEY,
  ...
  acknowledgeWarning: async (warningType: "IDLE" | "ABSOLUTE" | "SECURITY") => {
    try {
      ...
      const response = await apiInstance.post(
        "/api/auth/session/acknowledge-warning",
        { warningType },
        {
          withCredentials: true,
          headers: {
