...
import { 
  FaTimes, FaTicketAlt, FaHome, FaUser, FaBook, 
  FaChartBar, FaUsers, FaCog, FaServer, FaShieldAlt,
  FaClipboardList, FaUsersCog, FaTools, FaFileAlt,
  FaChevronDown, FaSignOutAlt, FaBell, FaMoon, FaSun, FaStar,
  FaSearch, FaHistory, FaFilter, FaEllipsisH,FaList
} from 'react-icons/fa';
...
const Sidebar: React.FC<SidebarProps> = ({ open, setOpen }) => {
  ...
  const navItems = useMemo(() => {
    ...
    const commonItems: NavItemType[] = [
      ...
      { 
        ...
        children: [
          ...
          { name: 'My Tickets', icon: FaTicketAlt, href: '/tickets/my-tickets' },
    ...
    const roleItems: Record<string, NavItemType[]> = {
      ...
      team_lead: [
        ...
        { name: 'Approvals', icon: FaClipboardList, href: '/approvals', badge: '3' },
      ],
      admin: [
        { 
          name: 'Analytics', 
          ...
          href: '/analytics',
          children: [
            { name: 'Dashboard', icon: FaChartBar, href: '/analytics/dashboard' },
            { name: 'Reports', icon: FaChartBar, href: '/analytics/reports' },
            { name: 'Metrics', icon: FaChartBar, href: '/analytics/metrics' },
          ]
        },
        { name: 'Customers', icon: FaUsers, href: '/customers' },
        { name: 'System Status', icon: FaServer, href: '/system-status' },
        ...
        { name: 'Audit Logs', icon: FaFileAlt, href: '/audit-logs' },
      ],
    };
  ...
  const renderNavItem = (item: NavItemType) => {
    ...
    const activeClass = 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-100 shadow-sm';
    const inactiveClass = 'text-gray-700 hover:bg-gray-100/70 dark:text-gray-300 dark:hover:bg-gray-700/50';
          ...
          <>
            ...
            >
              <div className="flex items-center">
                ...
                <span>{item.name}</span>
                {isRecent && (
                  <Badge variant="outline" className="ml-2 px-1 py-0 text-xs bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200">
                    Recent
                  </Badge>
                )}
                {item.badge && (
              ...
              <div className="flex items-center space-x-1.5">
                ...
                <FaChevronDown className={`h-3 w-3 transition-transform duration-200 text-gray-500 dark:text-gray-400 ${isExpanded ? 'transform rotate-180' : ''}`} />
              </div>
            </button>
            ...
            <NavLink
              ...
              className={({ isActive: routerIsActive }) => 
                `flex items-center justify-between px-4 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${
                  routerIsActive && item.name !== 'Dashboard' && item.name !== 'Profile'
                    ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-100 shadow-sm'
                    : 'text-gray-700 hover:bg-gray-100/70 dark:text-gray-300 dark:hover:bg-gray-700/50'
                }`
              }
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex items-center justify-between p-4 border-b border-gray-200/80 dark:border-gray-700/80">
            <div className="flex items-center">
              ...
              <span className="ml-3 text-lg font-semibold text-gray-900 dark:text-white">Support Hub</span>
            </div>
