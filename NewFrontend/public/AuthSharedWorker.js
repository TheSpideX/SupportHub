(() => {
  ...
  var Logger = class {
    constructor(component) {
      __publicField(this, "isDevelopment", import_meta.env.MODE === "development");
      __publicField(this, "SENSITIVE_FIELDS", ["password", "token", "secret", "credential", "auth"]);
      __publicField(this, "component");
      this.component = component || "App";
    }
    formatMessage(level, message, context = {}, category = "system", fileIdentifier) {
    ...
    trackPerformance(action, duration, context, fileIdentifier) {
      this.info(`Performance: ${action}`, __spreadProps(__spreadValues({}, context), {
        duration,
  ...
  function handleAuthStateChange(message) {
    var _a, _b;
    const isAuthenticated = (_a = message.payload) == null ? void 0 : _a.isAuthenticated;
    const isPriority = ((_b = message.payload) == null ? void 0 : _b.isPriority) === true;
    if (message.payload && isAuthenticated !== void 0) {
      const isLoginEvent = isAuthenticated === true && authState.isAuthenticated === false;
      const isLogoutEvent = isAuthenticated === false && authState.isAuthenticated === true;
      if (isLoginEvent || isLogoutEvent || isPriority) {
        ...
        authState = __spreadProps(__spreadValues(__spreadValues({}, authState), message.payload), {
          lastUpdate: Date.now(),
          updatedBy: message.tabId
        });
        broadcastToAllTabs(
