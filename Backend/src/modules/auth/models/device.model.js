...
DeviceSchema.path("hierarchyPath.userRoom").validate(function (value) {
  ...
  return (
    ...
    value.split(":")[1] === this.userId.toString()
  );
...
DeviceSchema.methods.removeSession = async function (sessionId) {
  ...
  if (deviceRoom) {
    ...
    const sessionRoomId = `session:${sessionId}`;
    deviceRoom.childRooms = deviceRoom.childRooms.filter(
      (id) => id !== sessionRoomId
    );
    await deviceRoom.save();
  }
...
DeviceSchema.methods.propagateEvent = async function (
  eventType,
  eventData,
  direction = "down"
) {
  ...
  if (direction === "up") {
    ...
    const userRoom = await Room.findOne({
      roomId: this.hierarchyPath.userRoom,
    });
