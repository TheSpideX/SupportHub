...
async function testDeviceFingerprinting() {
  try {
    ...
    console.log(
      `✅ Generated ${fingerprints.length} fingerprints, ${uniqueFingerprints.size} are unique`
    );
...
async function testTokenRefreshAndRotation() {
  try {
    ...
    try {
      ...
      console.log(
        "✅ Old refresh token is properly invalidated:",
        error.message
      );
    }
  ...
  try {
    ...
    if (tokens.length > 0 && tokens[0].accessToken) {
      try {
        ...
        console.log(
          "✅ Token is properly invalidated after logout:",
          error.message
        );
      }
    }
